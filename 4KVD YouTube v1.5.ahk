...
ClipName(TT1:="",OC1:=""){
  If TT1 && OC1
    TTL.Push(TT1),CHL.Push(OC1),CHK.Push(10)
  Loop % TTL.Count(){
    If CHK[A_Index]{
       CHK[A_Index]--
    }Else{
      ...
      If FileExist(DIR FIL){
        FileGetSize FSZ,% DIR FIL
        SIZ[A_Index]:=FSZ
      }
      If !FileExist(DIR PRT)
        If FileExist(DIR FIL){
          FileMove % DIR FIL,% DIR CHL[A_Index] " - " FIL
          TTL.Remove(A_Index),CHL.Remove(A_Index),SIZ.Remove(A_Index),CHK.Remove(A_Index)
        }
    }
   If TTL.Count()
     TMP.=TTL[A_Index] " : " CHL[A_Index] (TTL[A_Index]?"`n":"")
   Else
     TMP:=""
  }
  ToolTip % TTL.Count()?TMP:"",10,800,20
  SetTimer % A_ThisFunc,% TTL.Count()?-2000:"Off"
}
